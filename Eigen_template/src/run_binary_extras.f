...
!          b% s1% job% warn_run_star_extras = .false.
      ...
      integer function extras_binary_finish_step(binary_id)
         ...
         if (log10(abs(b% mtransfer_rate/(Msun/secyer))) >-10.0 .and.  &
            log10(abs(b% mtransfer_rate_older/(Msun/secyer))) <-10.0) then  
            b% s1% profile_data_prefix = 'profile_RLOF_start'
            b% s1% need_to_save_profiles_now = .true. 

         else if (log10(abs(b% mtransfer_rate_older/(Msun/secyer))) <-10.0 .and.  &
            log10(abs(b% mtransfer_rate/(Msun/secyer))) >-10.0) then  
            b% s1% profile_data_prefix = 'profile_RLOF_end'
            b% s1% need_to_save_profiles_now = .true. 
         else
            b% s1% profile_data_prefix = 'profile'
         endif
